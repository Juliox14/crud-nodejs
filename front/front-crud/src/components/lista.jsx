...
export const Lista = () => {
    ...
    useEffect(() => {
        const obtenerDatos = async () => {
            const res = await fetch("http://localhost:3001/getProductos");
    ...
    const eliminarProducto = async (id) => {
        ...
        await fetch("http://localhost:3001/eliminarProducto", {
            method: 'DELETE',
            headers: {
    ...
    const guardarCambios = async () => {
        if (producto.id) {
            setProductos(productos.map(item => (item.id === producto.id ? producto : item)));
            await fetch("http://localhost:3001/editarProducto", {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json'
                },
            ...
            await fetch("http://localhost:3001/agregarProducto", {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
        ...
        setProducto({ id: null, nombre: '', marca: '', categoria: '', precio: '' });
    };
    ...
    return (
        <div className="bg-gray-100 relative shadow-md sm:rounded-lg overflow-hidden pt-5 flex flex-col">
            {onEdit ? (
                <div className="flex items-center px-6 mb-5 gap-4">
                    <div className="flex gap-2 ">
                        <div className="flex flex-col gap-1">
                            <label className="text-xs text-gray-700 uppercase text-left font-bold">Nombre</label>
                            <input className="rounded-md text-sm px-1 box-border" type="text" defaultValue={producto.nombre} onChange={(e) => setProducto({ ...producto, nombre: e.target.value })} />
                        </div>
                        <div className="flex flex-col gap-1 text-left">
                            <label className="text-xs text-gray-700 uppercase text-left font-bold">Marca</label>
                            <input className="rounded-md text-sm px-1 box-border" type="text" defaultValue={producto.marca} onChange={(e) => setProducto({ ...producto, marca: e.target.value })} />
                        </div>
                        <div className="flex flex-col gap-1 text-left">
                            <label className="text-xs text-gray-700 uppercase text-left font-bold">Categoría</label>
                            <input className="rounded-md text-sm px-1 box-border" type="text" defaultValue={producto.categoria} onChange={(e) => setProducto({ ...producto, categoria: e.target.value })} />
                        </div>
                        <div className="flex flex-col gap-1 text-left">
                            <label className="text-xs text-gray-700 uppercase text-left font-bold">Precio</label>
                            <input className="rounded-md text-sm px-1 box-border" type="text" defaultValue={producto.precio} onChange={(e) => setProducto({ ...producto, precio: e.target.value })} />
                        </div>

                        <button className="flex flex-row justify-center items-end" onClick={() => guardarCambios()}>
                            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" strokeWidth="1.5" stroke="#1D4ED8" fill="none" strokeLinecap="round" strokeLinejoin="round">
                        ...
                        <button className="flex flex-row justify-center items-end" onClick={() => setOnEdit(false)}>
                            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" strokeWidth="1.5" stroke="#ff2825" fill="none" strokeLinecap="round" strokeLinejoin="round">
                                <path stroke="none" d="M0 0h24v24H0z" fill="none" />
                ...
                <div className="flex items-center justify-end px-6 mb-5 gap-4">
                    <button className="flex items-center justify-center w-auto h-auto bg-blue-600 p-2 text-white rounded-md" onClick={() => agregarProducto()}>
                                ...
                                <tr key={producto.id} className="bg-white border-b">
                                    ...
                                    <td className="px-6 py-4 text-gray-700">
                                        <div className="flex items-center gap-1">
                                            <button className="p-2  rounded-full  group transition-all duration-500  flex item-center" onClick={() => editarProducto(producto)}>
                                            ...
                                            <button className="p-2 rounded-full  group transition-all duration-500  flex item-center" onClick={() => eliminarProducto(producto.id)}>
